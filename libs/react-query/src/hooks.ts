/**
 * Hook factory functions for `@cleverbrush/react-query`.
 *
 * Each factory creates a hook function for a specific (group, endpoint)
 * pair.  The hooks delegate data fetching to the underlying
 * `@cleverbrush/web` typed client and automatically manage query keys.
 *
 * @module
 */

import type { WebError } from '@cleverbrush/web';
import {
    type QueryKey,
    useInfiniteQuery,
    useMutation,
