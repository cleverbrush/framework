    ...
    return (
        <div className="card">
            ...
            <p>
                ...
                specification. This means every schema you build is immediately
                usable in <strong>50+ libraries and frameworks</strong> without
                any adapter or wrapper — including tRPC, TanStack Form, React
            ...
            <p>
                All 13 builders expose a <code>[&apos;~standard&apos;]</code>{' '}
                getter on the base <code>SchemaBuilder</code> class. The
                property returns a cached object with three fields:
            </p>
            ...
            <h3>Usage</h3>
            <p>
                You can access <code>[&apos;~standard&apos;]</code> directly,
                but in practice you never need to — just pass a{' '}
...
// { issues: [{ message: 'minLength' }, { message: 'email' }] }`)
            ...
            <h3>TanStack Form</h3>
            <p>
                Pass any <code>@cleverbrush/schema</code> builder directly as a
                TanStack Form field validator — no adapter needed:
            </p>
            <pre>
                <code
                    // biome-ignore lint/security/noDangerouslySetInnerHtml: allow here
...
// />`)
            ...
            <h3>T3 Env</h3>
            <p>
                Use <code>@cleverbrush/schema</code> builders as T3 Env server
                and client environment validators:
            </p>
            <pre>
                <code
                    // biome-ignore lint/security/noDangerouslySetInnerHtml: allow here
  ...
  runtimeEnv: {
    ...
    NEXT_PUBLIC_API_URL: process.env.NEXT_PUBLIC_API_URL,
            ...
            <p>
                ...
                <code>@cleverbrush/schema</code> exposes its schemas <em>to</em>{' '}
                other tools via <code>[&apos;~standard&apos;]</code>, and it can{' '}
                <em>consume</em> schemas from other libraries via{' '}
                <code>extern()</code>. Wrap any Standard Schema v1 compatible
                schema (Zod, Valibot, ArkType, …) into a native builder — no
                rewriting needed:
            </p>
            <pre>
                <code
...
if (!result.valid) {
  ...
  console.log(zipErrors.errors);
            ...
            <p>
                <code>extern()</code> takes a single parameter — the external
                schema. Types and property descriptors are derived
                automatically. Validation is delegated to the external
                library&apos;s <code>[&apos;~standard&apos;].validate()</code>{' '}
                method, so <code>@cleverbrush/schema</code> never re-implements
                another library&apos;s validation logic.
            </p>

            <h3>Compatible tools</h3>
            <p>
                Any library that consumes Standard Schema v1 validators works
                with <code>@cleverbrush/schema</code> automatically:
            </p>
            <div className="table-wrap">
                <table className="api-table">
                    <thead>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="/showcases/t3-env">Live showcase →</a>
                            </td>
                        </tr>
                        <tr>
                            <td>tRPC</td>
                            <td>API / RPC</td>
                            <td>
                                Pass schema as procedure <code>.input()</code>
                            </td>
                        </tr>
                        <tr>
