...
export const metadata = {
    title: 'Migrating from Zod | @cleverbrush/schema',
    description:
        'Side-by-side API reference for developers moving from Zod to @cleverbrush/schema.'
};

export default function MigratingFromZodPage() {
    return (
        <div className="page">
            <div className="container">
                ...
                <div className="card">
                    ...
                    <p>
                        ...
                        full API surface — scroll to any section for runnable
                        code examples.
                    </p>
...
const PartialName = Base.partial('name');   // single field — no object wrapper needed`
                    ...
                    <p>
                        ...
                        <code>ObjectSchemaBuilder</code> directly, which lets
                        you compose schemas without listing property names:
                    </p>
                    <pre>
                        <code
                            // biome-ignore lint/security/noDangerouslySetInnerHtml: allow here
...
const LowerEmail = string().toLowerCase().email(); // same thing`
                    ...
                    <p>
                        ...
                        <code>.transform()</code> runs <em>after</em>{' '}
                        validation, changing the output type.{' '}
                        <code>.addPreprocessor()</code> runs <em>before</em>{' '}
                        validation, normalising the raw input. For most
                        real-world use cases (trim whitespace, normalise case,
                        coerce strings to numbers) the preprocessor approach is
...
console.log(User.introspect().isReadonly); // true`
                ...
                <div className="why-box">
                    ...
                    <p>
                        Zod schemas are opaque — you can validate data but
                        cannot inspect the schema&apos;s structure at runtime.{' '}
                        <code>@cleverbrush/schema</code> exposes a full
                        descriptor tree via <code>.introspect()</code>, making
                        schemas <strong>transparent and programmable</strong>.
                    </p>
                    <pre>
                        <code
                            // biome-ignore lint/security/noDangerouslySetInnerHtml: allow here
...
// Zod has no equivalent — introspection is architecturally impossible.`
                    ...
                    <p>
                        ...
                        ecosystem. <a href="/mapper">@cleverbrush/mapper</a>{' '}
                        uses PropertyDescriptors to provide type-safe property
                        selectors for object transformation.{' '}
                        <a href="/react-form">@cleverbrush/react-form</a> uses
                        them to auto-generate and auto-validate form fields.
                ...
                <div className="why-box">
                    ...
                    <p>
                        ...
                        <code>.refine()</code> — you can add a validator, but
                        you cannot add new <em>methods</em> to the builder with
                        proper TypeScript types.{' '}
                        <code>@cleverbrush/schema</code> has a first-class
                        extension system: add methods that appear on the builder
                        itself, fully typed, composable with everything else.
                    </p>
                    <pre>
                        <code
                            // biome-ignore lint/security/noDangerouslySetInnerHtml: allow here
...
const PostSlug = SlugString.slug().minLength(3).maxLength(60);`
                    ...
                    <p>
                        ...
                        <code>.unique()</code> methods are all implemented this
                        way — the extension system is used in production, not
                        just a theoretical feature.
                    </p>
                </div>
                ...
                <div className="card" id="gaps">
                    ...
                    <p>
                        ...
                        every Zod feature. Here is what&apos;s missing:
                    </p>
                    <div className="table-wrap">
                        <table className="api-table">
                            <thead>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        Use{' '}
                                        <code>object().intersect(other)</code>
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                ...
                                <tr>
                                    <td>
                                        <code>.transform()</code>{' '}
                                        (post-validation output type change)
                                    </td>
                                    <td>Partial</td>
                                    <td>
                                        <code>.addPreprocessor()</code> runs
                                        pre-validation; output type stays the
                                        same
                                    </td>
                                </tr>
                            </tbody>
                ...
                <div className="card">
                    ...
                    <ul>
                        <li>
                            ...
                            descriptor tree at runtime. Build code generators,
                            form renderers, API doc generators, and more —
                            directly from your schema.
                        </li>
                        <li>
                            <strong>Type-safe extension methods</strong> — Add
                            real builder methods with full TypeScript support.
                            Not just <code>.refine()</code> callbacks —
                            composable, discoverable, autocomplete-friendly
                            methods.
                        </li>
                        <li>
                            <strong>Schema-driven ecosystem</strong> —{' '}
                            <a href="/mapper">@cleverbrush/mapper</a> and{' '}
                            <a href="/react-form">@cleverbrush/react-form</a>{' '}
                            are built on the same schema foundation. One schema
                            definition drives validation, object mapping, and
                            form rendering simultaneously.
                        </li>
                        <li>
