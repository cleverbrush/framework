 ...
 *
 * `createClient()` wraps `@cleverbrush/web`'s `createClient()` and returns
 * a two-level proxy where each endpoint is **both** a callable function
 * (direct HTTP fetch) **and** an object with TanStack Query hooks
 * (`useQuery`, `useSuspenseQuery`, `useInfiniteQuery`, `useMutation`)
 * plus helper methods (`queryKey`, `prefetch`).
 *
 * @module
 */
...
import type { TypedQueryClient, UnifiedClient } from './types.js';
...
/**
 * @deprecated Use `createClient` instead. `createQueryClient` is kept for
 * backward compatibility and will be removed in a future major version.
 */
export function createQueryClient<T extends ApiContract>(
    contract: T,
    options?: ClientOptions
): TypedQueryClient<T> {
    return createClient(contract, options) as any;
}
