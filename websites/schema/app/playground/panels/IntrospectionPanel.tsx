            ...
            <div className="pg-panel pg-introspection-panel">
                ...
                <div className="pg-type-placeholder">
                    ...
                    <p className="pg-type-hint">
                        <code>.introspect()</code> reveals every validator,
                        preprocessor, extension, and constraint — making schemas
    ...
    return (
        <div className="pg-panel pg-introspection-panel">
            ...
            <div className="pg-introspection-callout">
                This is what makes @cleverbrush/schema unique — schemas are
...
function JsonTree({ data, depth = 0 }: { data: unknown; depth?: number }) {
    ...
    if (data === undefined)
        return <span className="pg-tree-null">undefined</span>;
    if (typeof data === 'string') {
        if (data === '[Function]')
            return <span className="pg-tree-fn">{data}</span>;
        return <span className="pg-tree-string">&quot;{data}&quot;</span>;
    }
    if (typeof data === 'number')
        return <span className="pg-tree-number">{data}</span>;
    if (typeof data === 'boolean')
        return <span className="pg-tree-boolean">{String(data)}</span>;

    if (Array.isArray(data)) {
        if (data.length === 0)
            return <span className="pg-tree-bracket">[]</span>;
...
function ObjectNode({
    ...
    return (
        <span>
            <button type="button" className="pg-tree-toggle" onClick={toggle}>
                {collapsed ? '▶' : '▼'}
            </button>
...
function getKeyClass(entries: [string, unknown][]): string {
    ...
    if (keys.includes('type') && keys.includes('validators'))
        return 'pg-tree-schema';
