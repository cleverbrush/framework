...
describe('string extensions', () => {
    ...
    describe('email()', () => {
        test('accepts a valid email', async () => {
            ...
            const result = await schema.validate('user@example.com');
            expect(result.valid).toBe(true);
            expect(result.object).toBe('user@example.com');
        });

        test('rejects a string without @', async () => {
            const result = await string().email().validate('not-an-email');
            expect(result.valid).toBe(false);
            expect(result.errors?.length).toBeGreaterThan(0);
        });

        test('rejects an empty string', async () => {
            const result = await string().email().validate('');
            expect(result.valid).toBe(false);
        });
        ...
        test('chains with built-in methods', async () => {
            ...
            const r1 = await schema.validate('a@b.co');
            expect(r1.valid).toBe(false);

            // valid email and long enough
            const r2 = await schema.validate('longuser@example.com');
    ...
    describe('url()', () => {
        test('accepts a valid https URL', async () => {
            const result = await string().url().validate('https://example.com');
            expect(result.valid).toBe(true);
        });

        test('accepts a valid http URL', async () => {
            const result = await string().url().validate('http://example.com');
            expect(result.valid).toBe(true);
        });

        test('rejects a non-URL string', async () => {
            const result = await string().url().validate('not a url');
            expect(result.valid).toBe(false);
        });

        test('rejects a disallowed protocol', async () => {
            const result = await string().url().validate('ftp://example.com');
            expect(result.valid).toBe(false);
        });

        test('accepts custom protocols', async () => {
            const schema = string().url({ protocols: ['ftp'] });
            const result = await schema.validate('ftp://files.example.com');
            expect(result.valid).toBe(true);
        });
    ...
    describe('uuid()', () => {
        ...
        test('rejects a non-UUID string', async () => {
            const result = await string().uuid().validate('not-a-uuid');
            expect(result.valid).toBe(false);
        });
        ...
        test('rejects truncated UUID', async () => {
            const result = await string()
                .uuid()
                .validate('550e8400-e29b-41d4-a716');
            expect(result.valid).toBe(false);
        });
    ...
    describe('ip()', () => {
        test('accepts a valid IPv4 address', async () => {
            const result = await string().ip().validate('192.168.1.1');
            expect(result.valid).toBe(true);
        });
        ...
        test('rejects an invalid IP', async () => {
            const result = await string().ip().validate('999.999.999.999');
            expect(result.valid).toBe(false);
        });

        test('rejects IPv6 when version is v4', async () => {
            const result = await string().ip({ version: 'v4' }).validate('::1');
            expect(result.valid).toBe(false);
        });
    ...
    describe('trim()', () => {
        test('trims whitespace from input', async () => {
            const result = await string().trim().validate('  hello  ');
            expect(result.valid).toBe(true);
        ...
        test('works before minLength validation', async () => {
            ...
            const r1 = await schema.validate('  hi  ');
            expect(r1.valid).toBe(false);
    ...
    describe('toLowerCase()', () => {
        test('lowercases the input', async () => {
            const result = await string().toLowerCase().validate('HELLO');
            expect(result.valid).toBe(true);
            expect(result.object).toBe('hello');
        });
    ...
    describe('nonempty()', () => {
        test('rejects an empty string', async () => {
            const result = await string().nonempty().validate('');
            expect(result.valid).toBe(false);
            expect(result.errors?.length).toBeGreaterThan(0);
        });

        test('accepts a non-empty string', async () => {
            const result = await string().nonempty().validate('a');
            expect(result.valid).toBe(true);
        });
